...
def transfer(
    ...
    assert len(from_address) == 20, "invalid from account length"
    assert len(to_address) == 20, "invalid to account length"
    # the parameter amount must be greater than or equal to 0. If not, this method should throw an exception.
    assert amount >= 0, "amount must be larger than 0"
...
def add_liquidity(
    ...
    """
    assert runtime.check_witness(user_address), "check witness failed"
...
def swap_tokens(
    ...
    token_a = get_token_a()
    token_b = get_token_b()
    assert token_in == token_a or token_in == token_b, "can only swap zNEO or zGAS"
