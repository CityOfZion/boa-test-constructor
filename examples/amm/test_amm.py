...
class AmmContractTest(SmartContractTestCase):
    ...
    async def asyncSetupClass(cls) -> None:
        ...
        await cls.transfer(
            Token.GAS, cls.genesis.script_hash, cls.owner.script_hash, 100
        )
        await cls.transfer(
            Token.GAS, cls.genesis.script_hash, cls.user.script_hash, 100
        )

        cls.contract_hash = await cls.deploy("./resources/amm.nef", cls.owner)
        cls.contract = GenericContract(cls.contract_hash)

        cls.zgas_contract_hash = await cls.deploy("./resources/wrapped_gas.nef", cls.genesis)
        cls.zgas_contract = GenericContract(cls.zgas_contract_hash)

        cls.zneo_contract_hash = await cls.deploy("./resources/wrapped_neo.nef", cls.genesis)
    ...
    async def test_01_set_address(self):
        ...
        set_address_wrong, _ = await self.call("set_address", [self.zneo_contract_hash, self.zgas_contract_hash],
                                               return_type=bool)
        self.assertFalse(set_address_wrong)

        # it will work now
        set_address_signed, _ = await self.call("set_address", [self.zneo_contract_hash, self.zgas_contract_hash],
        ...
        set_address_again, _ = await self.call("set_address", [self.zneo_contract_hash, self.zgas_contract_hash],
                                               return_type=bool)
    ...
    async def test_05_balance_of(self):
        balance_of_owner, _ = await self.call("balanceOf", [self.owner.script_hash], return_type=int)
    ...
    async def test_06_quote(self):
        ...
        quote, _ = await self.call("quote", [mock_amount_zneo, mock_reverse_zneo, mock_reverse_wgaz], return_type=int)
        self.assertEqual(mock_amount_zneo * mock_reverse_wgaz // mock_reverse_zneo, quote)

    async def test_07_on_nep17_payment(self):
        transferred_amount = 10
        await self.call("set_address", [self.zneo_contract_hash, self.zgas_contract_hash],
                        return_type=bool, signing_accounts=[self.owner])
        # adding the transferred_amount into user
        await self.transfer(
            Token.NEO, self.genesis.script_hash, self.zneo_contract_hash, transferred_amount
        )
        ...
        transfer_success = await self.call("transfer", [self.user.script_hash, self.contract_hash, transferred_amount, None],
                                           return_type=bool, target_contract=self.zneo_contract_hash)
        ...
        with self.assertRaises(AbortException):
            await self.call("onNEP17Payment", [self.user.script_hash, transferred_amount, None],
                            return_type=bool)
    ...
    async def test_08_add_liquidity(self):
        ...
        await self.call("set_address", [self.zneo_contract_hash, self.zgas_contract_hash],
                        return_type=bool, signing_accounts=[self.owner])
        await self.transfer(
            Token.NEO, self.genesis.script_hash, self.user.script_hash, mock_balance_zneo
        )
        await self.transfer(
            Token.GAS, self.genesis.script_hash, self.user.script_hash, mock_balance_zgas
        )

        # minting zNEO to user
        transfer_zneo = await self.transfer(
            Token.NEO, self.user.script_hash, self.zneo_contract_hash, mock_balance_zneo,
            signing_account=self.user
        )
        # minting zGAS to user
        transfer_zgas = await self.transfer(
            Token.GAS, self.user.script_hash, self.zgas_contract_hash, mock_balance_zgas,
            signing_account=self.user
        )
        ...
        approve_zneo, _ = await self.call("approve", [self.user.script_hash, self.contract_hash, mock_balance_zneo],
                                          return_type=bool, signing_accounts=[self.user], target_contract=self.zneo_contract_hash)
        # approving the AMM contract, so that it will be able to transfer zGAS from user
        approve_zgas, _ = await self.call("approve", [self.user.script_hash, self.contract_hash, mock_balance_zgas * zgas_decimals],
                                          return_type=bool, signing_accounts=[self.user], target_contract=self.zgas_contract_hash)
        ...
        balance_user_amm_before, _ = await self.call("balanceOf", [self.user.script_hash], return_type=int)
        reserves_before, _ = await self.call("get_reserves", return_type=list)
        balance_user_zneo_before, _ = await self.call("balanceOf", [self.user.script_hash], return_type=int, target_contract=self.zneo_contract_hash)
        balance_user_zgas_before, _ = await self.call("balanceOf", [self.user.script_hash], return_type=int, target_contract=self.zgas_contract_hash)
        balance_amm_zneo_before, _ = await self.call("balanceOf", [self.contract_hash], return_type=int, target_contract=self.zneo_contract_hash)
        balance_amm_zgas_before, _ = await self.call("balanceOf", [self.contract_hash], return_type=int, target_contract=self.zgas_contract_hash)

        liquidity = int(math.sqrt(transferred_amount_zneo * transferred_amount_zgas * zgas_decimals))

        # adding liquidity to the pool will give you AMM tokens in return
        add_liquidity_result, notifications = await self.call("add_liquidity", [transferred_amount_zneo, transferred_amount_zgas * zgas_decimals, 0, 0, self.user.script_hash],
        ...
        self.assertEqual([transferred_amount_zneo, transferred_amount_zgas * zgas_decimals, liquidity], add_liquidity_result)
        ...
        balance_user_amm_after, _ = await self.call("balanceOf", [self.user.script_hash], return_type=int)
        reserves_after, _ = await self.call("get_reserves", return_type=list)
        balance_user_zneo_after, _ = await self.call("balanceOf", [self.user.script_hash], return_type=int, target_contract=self.zneo_contract_hash)
        balance_user_zgas_after, _ = await self.call("balanceOf", [self.user.script_hash], return_type=int, target_contract=self.zgas_contract_hash)
        balance_amm_zneo_after, _ = await self.call("balanceOf", [self.contract_hash], return_type=int, target_contract=self.zneo_contract_hash)
        balance_amm_zgas_after, _ = await self.call("balanceOf", [self.contract_hash], return_type=int, target_contract=self.zgas_contract_hash)
        ...
        self.assertEqual(reserves_before[0].as_int() + transferred_amount_zneo, reserves_after[0].as_int())
        self.assertEqual(reserves_before[1].as_int() + transferred_amount_zgas * zgas_decimals, reserves_after[1].as_int())
        self.assertEqual(balance_user_zneo_before - transferred_amount_zneo, balance_user_zneo_after)
        self.assertEqual(balance_user_zgas_before - (transferred_amount_zgas * zgas_decimals), balance_user_zgas_after)
        ...
        # approving the AMM contract, so that it will be able to transfer zNEO from user
        approve_zneo, _ = await self.call("approve", [self.user.script_hash, self.contract_hash, transferred_amount_zneo],
                                          return_type=bool, target_contract=self.zneo_contract_hash, signing_accounts=[self.user])
        # approving the AMM contract, so that it will be able to transfer zGAS from user
        approve_zgas, _ = await self.call("approve", [self.user.script_hash, self.contract_hash, transferred_amount_zgas],
                                          return_type=bool, target_contract=self.zgas_contract_hash, signing_accounts=[self.user])
        self.assertTrue(approve_zneo)
        self.assertTrue(approve_zgas)
        ...
        transferred_amount_zgas_quoted = transferred_amount_zneo * reserves_before[1].as_int() // reserves_before[0].as_int()

        # since there are tokens in the pool already, liquidity will be calculated as follows
        liquidity = min(transferred_amount_zneo * total_supply_before // reserves_before[0].as_int(),
                        transferred_amount_zgas_quoted * total_supply_before // reserves_before[1].as_int())
        for index, result in enumerate(add_liquidity_result):
            add_liquidity_result[index] = result.as_int()
        self.assertEqual([transferred_amount_zneo, transferred_amount_zgas_quoted, liquidity], add_liquidity_result)
        ...
        self.assertEqual(2, len(sync_events[0].state.as_list()))
        balance_zneo, balance_zgas = sync_events[0].state.as_list()
        self.assertEqual(reserves_before[0].as_int() + transferred_amount_zneo, balance_zneo.as_int())
        self.assertEqual(reserves_before[1].as_int() + transferred_amount_zgas_quoted, balance_zgas.as_int())
    ...
    async def test_09_remove_liquidity(self):
        ...
        set_address = await self.call("set_address", [self.zneo_contract_hash, self.zgas_contract_hash],
                                      return_type=bool, signing_accounts=[self.owner])
        ...
        transfer_neo = await self.transfer(
            Token.NEO, self.genesis.script_hash, self.user.script_hash, test_balance_zneo
        )
        transfer_gas = await self.transfer(
            Token.GAS, self.genesis.script_hash, self.user.script_hash, test_balance_zgas // zgas_decimals
        )
        ...
        # minting zNEO to user
        transfer_zneo = await self.transfer(
            Token.NEO, self.user.script_hash, self.zneo_contract_hash, test_balance_zneo,
            signing_account=self.user
        )
        # minting zGAS to user
        transfer_zgas = await self.transfer(
            Token.GAS, self.user.script_hash, self.zgas_contract_hash, test_balance_zgas // zgas_decimals,
            signing_account=self.user
        )
        self.assertTrue(transfer_zneo)
        self.assertTrue(transfer_zgas)
        ...
        # adding liquidity to the pool will give you AMM tokens in return
        add_liquidity_result, _ = await self.call("add_liquidity", [transferred_amount_zneo, transferred_amount_zgas, 0, 0, self.user.script_hash],
                                                  return_type=list, signing_accounts=[self.user])
        ...
        remove_liquidity, notifications = await self.call("remove_liquidity", [liquidity, 0, 0, self.user.script_hash],
                                                          return_type=list, signing_accounts=[self.user])
        ...
        self.assertEqual(2, len(sync_events[0].state.as_list()))
        balance_zneo, balance_zgas = sync_events[0].state.as_list()
        self.assertEqual(reserves_before[0].as_int() - zneo_refunded, balance_zneo.as_int())
        self.assertEqual(reserves_before[1].as_int() - zgas_refunded, balance_zgas.as_int())
        ...
        self.assertEqual(total_supply_before - liquidity, total_supply_after)
        self.assertEqual(balance_user_amm_before - liquidity, balance_user_amm_after)
        self.assertEqual(reserves_before[0].as_int() - zneo_refunded, reserves_after[0].as_int())
        self.assertEqual(reserves_before[1].as_int() - zgas_refunded, reserves_after[1].as_int())
        self.assertEqual(balance_user_zneo_before + zneo_refunded, balance_user_zneo_after)
        self.assertEqual(balance_user_zgas_before + zgas_refunded, balance_user_zgas_after)
        self.assertEqual(reserves_before[0].as_int(), balance_amm_zneo_before)
        self.assertEqual(reserves_before[1].as_int(), balance_amm_zgas_before)
        self.assertEqual(reserves_after[0].as_int(), balance_amm_zneo_after)
        self.assertEqual(reserves_after[1].as_int(), balance_amm_zgas_after)
    ...
    async def test_10_swap_zneo_to_zgas(self):
        ...
        # adding liquidity to the pool will give you AMM tokens in return
        add_liquidity, _ = await self.call("add_liquidity", [transferred_amount_zneo, transferred_amount_zgas, 0, 0, self.user.script_hash],
                                           return_type=list, signing_accounts=[self.user])
        ...
        # approving the AMM contract, so that it will be able to transfer zNEO from user
        approve_zneo, _ = await self.call("approve", [self.user.script_hash, self.contract_hash, swapped_zneo],
                                          return_type=bool, signing_accounts=[self.user], target_contract=self.zneo_contract_hash)
        self.assertTrue(approve_zneo)
        ...
        swap_tokens, notifications = await self.call("swap_tokens", [swapped_zneo, 0, self.zneo_contract_hash, self.user.script_hash],
                                                     return_type=int, signing_accounts=[self.user])

        # there is a 0.3% fee when doing a swap
        swapped_zneo_with_fee = swapped_zneo * (1000 - 3)
        swapped_zgas = swapped_zneo_with_fee * reserves_before[1].as_int() // (
        ...
        address, amount_zneo_in, amount_zgas_in, amount_zneo_out, amount_zgas_out = swap_events[0].state.as_list()
        self.assertEqual(self.user.script_hash, address.as_uint160())
        ...
        self.assertEqual(reserves_before[0].as_int() + swapped_zneo, reserves_after[0].as_int())
        self.assertEqual(reserves_before[1].as_int() - swapped_zgas, reserves_after[1].as_int())
    ...
    async def test_11_swap_zgas_to_zneo(self):
        ...
        # approving the AMM contract, so that it will be able to transfer zGAS from user
        approve_zneo, _ = await self.call("approve", [self.user.script_hash, self.contract_hash, swapped_zgas],
                                          return_type=bool, signing_accounts=[self.user], target_contract=self.zgas_contract_hash)
        self.assertTrue(approve_zneo)
        ...
        swap_tokens, notifications = await self.call("swap_tokens", [swapped_zgas, 0, self.zgas_contract_hash, self.user.script_hash],
                                                     return_type=int, signing_accounts=[self.user])

        # there is a 0.3% fee when doing a swap
        swapped_zgas_with_fee = swapped_zgas * (1000 - 3)
        swapped_zneo = swapped_zgas_with_fee * reserves_before[0].as_int() // (reserves_before[1].as_int() * 1000 + swapped_zgas_with_fee)
